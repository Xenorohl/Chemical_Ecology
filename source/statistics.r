...
test = log_data[, colSums(log_data) > 0]
pca_test = PCA(test, graph = FALSE)
...
##### analysis on only some of the compounds 
...
log_new = log_data %>%
    select(
        "X2.Hexenal", 
        "Z.3.Hexenyl.acetate.aldrich.", 
        "linalool.....Calculated.", 
        ".beta..Myrcene", 
        "R....LIMONENE...Calculated.", 
        "Geraniol", 
        "Benzaldehyde...Calculated.", 
        "X.E..BETA.farnesene...Calculated."
    )
...
# Ajouter les groupes
abundance_long_new <- abundance_long_new %>%
  left_join(sample_group, by = c("Sample" = "Sample"))

x11()
ggplot(abundance_long_new,
       aes(x = Group,
           y = Abundance,
           fill = Group)) +
  ...
  labs(
    title = "Abundances of interesting volatile compounds",
    subtitle = "Control vs Treatment",
    x = "Groupe",
    y = "log10(Abondance + 1)"
  )  + 
...
# quartz.save(
#   "./plots/abundance_gcms_interesting.png",
#   type = "png",
#   dpi = 300,
#   width = 20,
#   height = 12
...
### statistical tests on the interesting compounds

farne_control = log_new$`(E)-β-farnesene`[sample_group$Group == "Control"]
farne_treat = log_new$`(E)-β-farnesene`[sample_group$Group == "Treatment"]

hexa_control = log_new$`2-Hexenal`[sample_group$Group == "Control"]
hexa_treat = log_new$`2-Hexenal`[sample_group$Group == "Treatment"]

benza_control = log_new$Benzaldehyde[ sample_group$Group == "Control"]
benza_treat = log_new$Benzaldehyde[ sample_group$Group == "Treatment"]

geraniol_control = log_new$Geraniol[ sample_group$Group == "Control"]
geraniol_treat = log_new$Geraniol[ sample_group$Group == "Treatment"]   

limonene_control = log_new$Limonene[ sample_group$Group == "Control"]
limonene_treat = log_new$Limonene[ sample_group$Group == "Treatment"]   

linalool_control = log_new$Linalool[ sample_group$Group == "Control"]
linalool_treat = log_new$Linalool[ sample_group$Group == "Treatment"]   

hexenyl_control = log_new$`Z-3-Hexenyl acetate`[ sample_group$Group == "Control"]
hexenyl_treat = log_new$`Z-3-Hexenyl acetate`[ sample_group$Group == "Treatment"]

myrcene_control = log_new$`β-Myrcene`[ sample_group$Group == "Control"]
myrcene_treat = log_new$`β-Myrcene`[ sample_group$Group == "Treatment"] 
